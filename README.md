...
[![Version: Beta](https://img.shields.io/badge/version-0.3.0--beta-orange.svg)](VERSION)
...
- **バックグラウンド転送**: 大容量ファイルの非同期転送システム
- **循環スクロール**: ファイル一覧で先頭↔末尾のループ移動
- **直感ナビゲーション**: 矢印キーでの親ディレクトリ移動
- **Kキーディレクトリ作成**: 簡単な新規ディレクトリ作成
...
| キー | 動作 |
|------|------|
| `Ctrl+P`, `↑` | カーソル上移動（先頭で末尾にループ） |
| `Ctrl+N`, `↓` | カーソル下移動（末尾で先頭にループ） |
| `Ctrl+A` | 左ペイン移動 |
| `Ctrl+B`, `←` | 左ペイン：親ディレクトリ、右ペイン：左ペイン移動 |
| `Ctrl+E` | 右ペイン移動 |
| `Ctrl+F`, `→` | 右ペイン：親ディレクトリ、左ペイン：右ペイン移動 |
| `TAB` | ペイン切り替え (循環) |
...
**注意**: 100MB以上のファイルやディレクトリの操作時にバックグラウンド転送オプションが自動表示されます。
